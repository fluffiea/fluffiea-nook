...
> [!tip] 区分 mount 和 umount
> `mount` 是把 U 盘/硬盘"接入"到某个目录，`umount` 是断开连接。u**m**ount 少写一个 n，容易记错。
...
> [!tip] LVM 关系链
> **硬盘 → 分区 → 物理卷(pv) → 卷组(vg) → 逻辑卷(lv) → 文件系统**
...
sudo docker compose up -d      # 启动容器（-d = detach 后台运行）
...
Linux 文件系统标准（FHS）规定了不同类型数据的存放位置。
...
|:----|:-------|:---------|
| `/` | 根目录，文件系统的起点 | — |
| `/home/` | 用户个人文件 | 文档、项目代码、下载 |
| `/opt/` | **第三方软件包（最常用）** | Immich、frp、手动部署的服务 |
| `/srv/` | 服务数据 | Web 站点文件、Git 裸仓库 |
| `/etc/` | 系统配置文件 | nginx 配置、系统服务配置 |
| `/var/log/` | 日志文件 | 系统日志、应用日志 |
| `/var/lib/` | 包管理器数据 | apt 装的东西，**别手动往里放** |
| `/mnt/` | 临时挂载外部设备 | U 盘、移动硬盘 |
| `/boot/` | 系统启动文件、内核 | — |
...
| ❌ 错误做法 | 说明 |
|:-----------|:-----|
| 把服务放 `/home/你/` 下 | 跟用户目录耦合太紧，别人访问不方便 |
| 往 `/var/lib/` 里手动创建文件 | 那是给 apt 用的，容易跟系统文件混淆 |
| 放 `/root/` 下 | root 用户的家目录，非 root 用户操作不便 |

> [!tip] 最佳实践
> **自己部署的服务都放 `/opt/<项目名>/`**，整洁、互不影响、一眼看出是手动装的。

> [!tip] 学习建议
> 不要死记硬背，在实际操作中遇到问题就用 `man <cmd>` 查手册，或者 `<cmd> --help` 看简版帮助。用多了自然就记住了！
